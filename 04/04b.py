...
class Bingo:
    ...
    def play(self):
        ...
        for i, board in enumerate(self.boards):
            if not (i in self.winningBoards):
                for j, row in enumerate(board):
                    for k, number in enumerate(row):
        ...
        roundWinningBoards = self.checkWinningBoards(boardsToCheck)
        if len(roundWinningBoards) > 0:
            print("Winning boards")
            print("==============")
            for index in roundWinningBoards:
                board = self.boards[index]
                score = self.calculateWinningBoardScore(board)
                print("Score: ", score)
                self.printBoard(board)
                print()

        self.winningBoards.update(roundWinningBoards)
...
while len(bingo.getWinningBoards()) < len(bingo.boards):
    bingo.play()
